...
### 4_실수형간의 형변환
  * float에서 double으로 변환하는 경우 지수는 float의 기저인 127을 빼고 double의 기저인 1023을 더해서 변환, 가수는 float의 23자리 가수를 채우고 나머지는 0으로 채운다.
  * double에서 float로 변환하는 경우 위와는 반대로 지수는 1023을빼고 127을 더한다,가수는 23자리만 저장후 모두 버린다.

### 5_정수형과 실수형 간의 현변환
* 실수 -> 정수 : 소수점 이하는 버려진고 저장된다.
* 정수 -> 실수 : 정수를 2진수로 변환후 정규화를거쳐 실수의 저장방식으로 저장

### 6_자동형변환
* 서로 다른 타입간의 대입이나 연산을할 때 형변환으로 타입을 일치시키는 것이 원직이지만 생략가능하다.   
	ex) float f = 1234;
* 정수를 float에 저장할때 형변환을 생략하면 컴파일러가 자동으로 형변환을 해준다
#### 형변화 규칙   
1. 기존의 값을 최대한 보존할 수 있는 타입으로 자동 형변환한다.
2. boolean을 제외한 나머지 7개의 기본형은 서로 형변환이 가능하다.
3. 기본형과 참조형은 서로 형변환할수 없다.
4. 서로 다른 타입의 변수간의 연산은 형변환을 하는 것이 원칙이지만,   
